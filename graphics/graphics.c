...
Image *scaleImage(Image *image, float sW, float sH)
{
    ...
    for (y = 0; y < image->imageHeight; y++) {
        for (x = 0; x < image->imageWidth; x++) {
            out->data[(int)((x*sW) + (y*sH) * out->textureWidth)] |= image->data[(int)(x + y * image->textureWidth)];
        }
    }
...
void blitScaleToScreen(Image* source, int dx, int dy, float sW, float sH)
{
        if (!initialized) return;
        ...
        while (j < source->imageWidth) {
                ...
                vertices[1].x = dx + j*sW + sliceWidth*sW;
